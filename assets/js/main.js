...
if (!OWM_API_KEY) {
  console.error(
    "Missing OpenWeatherMap API key - check GitHub Secrets configuration"
  );
}
...
function initializeMap() {
  ...
  console.log(
    `Setting initial zoom level: ${initialZoom} (mobile: ${
  ...
  try {
    ...
    if (data.wind && data.wind.deg !== undefined) {
      ...
      document.getElementById(
        "windDirection"
      ).textContent = `${compassDirection} (${Math.round(data.wind.deg)}°)`;
    } else {
    ...
    if (window.timelineControls) {
      window.timelineControls.markDataAsUpdated();
    }
...
function displayWeatherError(message = "Unable to load data") {
  ...
  document.getElementById(
    "forecast"
  ).innerHTML = `<div style="color: #ef4444; font-size: 12px;">${message}</div>`;
}
...
async function loadRadar() {
  ...
  try {
    ...
    if (
      ...
      data.radar &&
      data.radar.nowcast &&
      Array.isArray(data.radar.nowcast) &&
      data.radar.nowcast.length > 0
    ) {
    ...
    if (radarTimestamp) {
      ...
      // Update last updated timestamp for radar
      if (window.timelineControls) {
        window.timelineControls.markDataAsUpdated();
      }
    } else {
...
// Make loadRadar available globally for timeline controls
...
function addWeatherLayers() {
  if (!OWM_API_KEY || !map) {
    console.warn(
      "Skipping OpenWeatherMap layers - API key not configured or map not initialized"
    );
    return;
  }
...
async function initialize() {
  ...
  console.log(
    "Environment: GitHub Pages deployment with secure API key injection"
  );
  ...
  try {
    ...
    // Load initial data
    await Promise.all([loadRadar(), loadWeatherData()]);
    ...
    console.log(
      `Auto-refresh enabled: ${AUTO_REFRESH_INTERVAL / 1000 / 60} minutes`
    );
  } catch (error) {
